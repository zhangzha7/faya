  ...
  <view class="wrap">
    <view class="side">
      <ul>
        ...
        <li :class="{current:flag==item.id,liInfo}" v-for="item in xueke" :key="item.id" @click="changeActive(item)">{{item.name}}</li>
...
export default {
  onLoad(key){
    ...
    this.reqXueke(JSON.parse(key.option).majorId)
    this.flag=JSON.parse(key.option).id
    this.active=JSON.parse(key.option).name
    this.getMajorItemVo(JSON.parse(key.option).classId)
  },
  data(){
    return{
      active:'',
  ...
  methods:{
    go(itemId){
      uni.navigateTo({
        url:"/pages/shouye/zhuanyeku/detail?option="+itemId+"&className="+this.active
      })
    },
    changeActive(item){
      this.getMajorItemVo(item.classId)
    ...
    getMajorItemVo(classId){
      uni.showLoading({
        title:"加载中"
      })
      uni.request({
        url: `${this.base.url}/api/home/majorclass/getMajorItemVo`,
        method:'GET',
        data:{
